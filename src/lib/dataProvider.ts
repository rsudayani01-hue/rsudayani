...
export const dataProvider: DataProvider = {
    getList: async ({ resource, pagination, filters, sorters }) => {
        ...
        if (filters) filters.forEach((f: any) => { if (f.field) query = query.eq(f.field, f.value); });
        if (sorters?.length) query = query.order(sorters[0].field, { ascending: sorters[0].order === 'asc' });
    ...
    update: async ({ resource, id, variables }) => {
        const { data, error } = await supabase.from(resource).update(variables as any).eq('id', id).select().single();
        if (error) throw error;
        return { data };
    },
    ...
    custom: async ({ url, method, filters }) => {
        ...
        if (method === "get" || !method) {
            let query = supabase.from(url).select('*');
